...
def main(argv): 
    ...
    if len(argv)>1:
        ...
        # print "neighRadii : %f" %neighRadii
        # print "cohW : %f " %(cohW*100)
        # print "repW : %f" %(repW*100)
        # print "alignW : %f" %(alignW*100)
        # print "crowdingThr : %f" %crowdingThr
        # print "leaderW : %f" %leaderW        
        # print "visualON : %s" %visualON
        # print "timeOutT : %f" %timeOutT        
        # # raw_input ("... Start Simulation - press enter to continue ...")
        # print "\n"
    else: 
    ...
    if(visualON): scenario = pygame.display.set_mode(scenarioSize) # Configure pygame window    
    ...
    strip = width/5 # Strip among which the init.pos of casual agent is going to be randomlly selected
    ...
    "Dynamic parameters"   
    maxVel = 3.0*damp  # Max. delta speed that can be performed on each time step
    ...
    casualAgentR = 10.0 # Artificial "Size" of casual agents and Radii among which a collision is determined
    ...
    specialRFact = 3 #8 Factor which increases the Radii at which Special Agents are considerder neighbors
    ...
    for i in range(noBoids):
        boids.append(Boid(0,strip)) # casual agent 
    ...
    while ellapsedT<=timeOutT and not(evoSuccess):
        ...
        centroidAgent = Boid(0,0) # trick used to be able to recycle Euclidian distance to boids method of Boid Class
        ...
        if (visualON):          
            ...
            sceneColor, varC = updateBG(sceneColor,sceneColorOri,deltC,varC)        
            ...
            if(medGRadiiV_PerIter>w):        
                pygame.draw.circle(scenario, (255,255,255), groupC, 8, 0)
                pygame.draw.circle(scenario, (255,255,255), groupC, int(medGRadiiV_PerIter), w)
            ...
            for boid in boids:
                ...
                #     pygame.draw.circle(scenario, (255,0,0), (int(boid.x), int(boid.y)), 7, 0)
                    ...
                    if boid not in boidsNotATgoal: colorRing = (0,0,0)
                    else:  colorRing = (255,0,0) 
                    pygame.draw.circle(scenario, colorRing, (int(boid.x), int(boid.y)), int(casualAgentR),3)
                    
                    # Draw radii line
                    pygame.draw.line(scenario, (102,178,255), groupC, (int(boid.x), int(boid.y)), 2)
        ...
        # if mouse[0]: pygame.time.delay(250) # pass
        # else:
        #     print "PAUSED"
        #     while not(mouse[0]):
        #         for event in pygame.event.get():
        ...
        [ boid.move(maxVel) for boid in boids if (boid.type == 0 and boid not in boidsAtgoal)]  
    ...
    medGRadiiV = 1.0/((medGRadiiV/numIter) + 1) # --> insideRadV /=numIter # Average median raddi ver  No. of position updates (iterations)
    if math.isnan(medGRadiiV) : medGRadiiV = 0.0

    if deltaTV == 0.0: deltaTV = 0.0
    else: deltaTV = 1.0/( ((deltaTV/factUS)/(numATgoal+1)) + 1)  # Average deltaTimes over  No. of arrivals to goal --> Precisely, it should ve over(numATgoal-1) , 
    ...
    fitVals = np.array([[interCollV, medGRadiiV, deltaTV, t2GoalV,numATgoalV,evoSuccessV]])
    return fitVals
